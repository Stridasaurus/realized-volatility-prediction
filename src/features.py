...
def make_labels(
    targets: pd.DataFrame, target: str, horizon: int
    ...
    if horizon not in VALID_HORIZONS:
        raise ValueError(
            f"horizon must be one of {VALID_HORIZONS} (frozen, manifesto s8), "
            f"got {horizon!r}"
        )
    s = targets[target]
    if horizon == 1:
    ...
    if len(nonpos):
        raise ValueError(
            f"non-positive label (log-target undefined) on {list(nonpos[:10])}"
        )
    return y, np.log(y)
    ...
    *,
    ...
    for tier in tiers:
        ...
        for name, series in cols.items():
            if series.isna().all():
                raise ValueError(
                    f"tier {tier} column {name} has no data in the snapshot"
                )
...
class TrainOnlyScaler:
    ...
    def fit(self, X: pd.DataFrame, train_idx: pd.DatetimeIndex) -> "TrainOnlyScaler":
        ...
        if zero:
            raise ValueError(
                f"zero-variance feature(s) on the fit span: {zero} — fix or drop "
...
def _perturb_after(
    snap: Snapshot, t: pd.Timestamp, rng: np.random.Generator
) -> Snapshot:
    spy = snap.spy.copy()
    vix = snap.vix.copy()
...
def leakage_probe(
    ...
    *,
    ...
    """
    base = build_features(
        build_targets(snap), snap, target=target, horizon=horizon, tiers=tiers
    )
    snap_p = _perturb_after(snap, t, rng)
    pert = build_features(
        build_targets(snap_p), snap_p, target=target, horizon=horizon, tiers=tiers
    )
    ...
    try:
        pd.testing.assert_series_equal(
            base.y.loc[label_safe], pert.y.reindex(label_safe), check_exact=True
        )
    except AssertionError:
        return False
    return True
